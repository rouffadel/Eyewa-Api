...
namespace Eyewa_new_api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpPost("LoginWithIdentity")]
        [AllowAnonymous]
        public async Task<IActionResult> LoginWithIdentity(string username, string password)
        {
            string ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString() ?? "127.0.0.1";
            var result = await _authService.LoginWithIdentity(username, password, ipAddress);
            if (result.Status == "200")
                return Ok(result);
            return BadRequest(result);
        }
